...
def format_report(writer, df, sheet_name):
    ...
    sheet.insert_rows(3)
    sheet["A1"] = "GST Data"
    sheet["A2"] = "Columns: " + ", ".join(df.columns[:len(df.columns)//2])
    sheet["A4"] = "Tally Data"
    sheet["A5"] = "Columns: " + ", ".join(df.columns[len(df.columns)//2:])
    sheet["A1"].font = bold_font
    sheet["A4"].font = bold_font

    # Insert space between sections
    sheet.insert_rows(max_row + 1)
    sheet.insert_rows(max_row + 2)
    sheet["A" + str(max_row + 2)] = "Difference between GST and Tally Data"
    sheet["A" + str(max_row + 2)].font = bold_font

    # Apply formatting
    for col in sheet.iter_cols(min_row=1, max_row=max_row + 2, min_col=1, max_col=max_col):
        for cell in col:
...
if report_type == "GST Reconciliation":
    ...
    def generate_gst_report(tally_file, gstr_file):
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            format_report(writer, combined_df, "GSTR-2B")
...
# Debit Note Reconciliation Report
    ...
    def generate_debit_note_report(debit_file, gstr_file):
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            format_report(writer, output_df, "Sheet1")
...
# Combined GST Reconciliation Report
    ...
    def generate_combined_report(tally_file, gstr_file, debit_file):
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            ...
            format_report(writer, combined_df, "Sheet1")
